...
### BẢNG TỔNG HỢP — CÁC THIẾT BỊ ĐẦU VÀO (Input Devices)

⚠️ Bảng này là nguồn ôn chính cho dạng bài "phân biệt chức năng thiết bị đầu vào" và "nối thiết bị với mô tả".

| Thiết bị | Chức năng chính | Cách hoạt động |
|---|---|---|
| **Keyboard** | Nhập liệu văn bản và các lệnh chữ | Nhấn các phím vật lý |
| **Mouse** | Di chuyển con trỏ, nhấp chọn và kéo thả | Dùng cảm biến quang học hoặc bi lăn |
| **Light Pen** | Vẽ hoặc chọn trực tiếp trên màn hình CRT | Phát hiện ánh sáng phát ra từ màn hình |
| **Graphics Tablet** | Thiết kế đồ họa, vẽ phác thảo kỹ thuật số | Dùng bút stylus viết trên bàn cảm ứng |
| **Trackball** | Di chuyển con trỏ chuột thay cho chuột | Xoay quả bi cố định bằng ngón tay |
| **Microphone** | Ghi âm giọng nói, nhập lệnh bằng tiếng nói | Chuyển đổi sóng âm sang tín hiệu điện số |
| **Scanner** | Chụp tài liệu giấy chuyển thành ảnh số | Quét quét cảm biến hình ảnh trên giấy |
...
Khi viết mô tả kỹ thuật cho thiết bị máy tính, ta sử dụng các cấu trúc chỉ mục đích và chức năng sau:

| Cấu trúc mô tả | Công thức | Ví dụ thực tế |
|---|---|---|
| **for + V-ing** | **device + for + V-ing** | This is a device **for controlling** the cursor. |
| **used to + V** | **used + to + V** | A keyboard is **used to enter** text into the computer. |
| **Relative clause** | **device + which/that + V** | A scanner is a device **which captures** images from paper. |
| **Passive Relative clause** | **device + which/that + is/are used to + V** | This is a tool **that is used to draw** graphics. |
| **work by + V-ing** | **work/operate + by + V-ing** | A light pen **works by detecting** light from the screen. |

Ngoài ra, khi mô tả đặc điểm phần cứng (features), ta dùng:
- **feature / features + N:** mang đặc điểm gì (Ví dụ: *The mouse features two buttons and a wheel.*)
- **allow/enable + O + to + V:** cho phép ai/cái gì làm gì (Ví dụ: *It allows designers to draw naturally.*)
- **be equipped with + N:** được trang bị bộ phận gì (Ví dụ: *Laptops are equipped with a touchpad.*)
...
1. A device with keys used to type text and input data.
2. A small handheld device with buttons moved over a flat surface to control the cursor.
3. A pen-like light-sensitive device used to select items directly on a CRT screen.
4. A device containing a ball on top which is rolled with fingers to move the pointer.
5. An input device used to digitize paper documents and photos into computer files.
6. A board with a digital stylus that allows natural drawing on a computer.
...
3. A light pen works by ________________ (detect) light signals from the computer monitor.
...
3. It works in combination with other keys. For example, you press this key and C to copy text.
4. It removes the character to the left of the cursor or any selected text.
5. It produces UPPER CASE characters (when held down).
6. It locks the keyboard in capital letter mode (does not affect numbers/symbols).
7. It moves the cursor horizontally to the right for a fixed number of spaces.
8. They are used to move the cursor up, down, left, and right.
...
4. **g** (backspace) — Phím xóa ký tự bên trái con trỏ.
5. **d** (shift) — Phím shift viết hoa tạm thời khi giữ.
6. **c** (Caps Lock) — Phím viết hoa toàn bộ ký tự chữ cái.
...
4. **select** — bôi đen/chọn văn bản.
5. **drag** — kéo rê hình ảnh sang vị trí mới.
...
*   **2-C** (Alphanumeric keys — Các phím chứa ký tự chữ cái và số thường dùng)
*   **3-A** (Cursor control keys — Các phím mũi tên điều khiển hướng)
