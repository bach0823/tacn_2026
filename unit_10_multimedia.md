...
> 📌 **Tóm tắt:** Ứng dụng đa phương tiện (multimedia) được dùng rộng rãi trong các lĩnh vực như bảo tàng, ngân hàng, giáo dục nhờ khả năng truyền tải thông tin bằng hình ảnh và âm thanh sống động. Sức mạnh của phần mềm đa phương tiện nằm ở siêu văn bản (hypertext), siêu đa phương tiện (hypermedia) và tính tương tác (interactivity).
...
> 📌 **Tóm tắt:** Chỉ cần có card âm thanh (sound card), máy tính có thể thu và phát âm thanh. Giao diện kỹ thuật số `MIDI` giúp nhạc cụ điện tử kết nối với máy tính, và phần mềm `DAW` hỗ trợ trộn nhạc. Định dạng `MP3` nén dữ liệu âm thanh và được tạo ra nhờ phần mềm trích xuất `CD ripper`. Công nghệ truyền phát âm thanh trực tuyến (streaming audio) và truyền phát sự kiện (webcast) yêu cầu các phần mềm hỗ trợ cắm thêm (plug-in) như *RealPlayer* hoặc *QuickTime*.
...
```mermaid
graph LR
  A[hypertext] --- 1["the process of manipulating video images"]
  B[hypermedia] --- 2["text with links which take you to other pages"]
  C[streaming] --- 3["a technique for playing sound and video files while they're downloading"]
  D[webcast] --- 4["a live event broadcast over the Internet"]
  E[video editing] --- 5["a form of enriched multimedia which allows an interactive and dynamic linking of visual and audio elements"]
```
...
**Câu 2 (Điền từ):** A software program that extracts music tracks from audio CDs and saves them as MP3s is called a ____________. *(CD ripper)*
...
**Câu 4 (Điền từ):** Fill in the blank with the correct verb form: "We won't be able to listen to the webcast unless we _________ (have) the correct plug-in." *(have)*

**Câu 5 (Điền từ):** What does `MP3` stand for? _____________. *(MPEG audio player 3)*
...
### Exercise D — Second Conditional Practice (Gợi ý trả lời)
